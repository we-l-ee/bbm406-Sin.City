...
def load_models(pathimport feature_extraction as feature
import classifiers as cls
import os
...
def preprocessing():
    ...
    x_train, y_train = feature.extract_features(x_train, y_train)
    feature.save_data("1", x_train, y_train)

def main(classifiers, create_data = True, train_data=True):
    if create_data:
        preprocessing(classifiers)
    classifiers = cls.get_model(classifiers)

    x_train, y_train = feature.load([1])
    if train_data:
        pass
