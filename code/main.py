...
def test(models, test_type, **kwargs):
    ...
    all_preds, y_test = cls.np.array([model.test(test_type, **kwargs) for model in models])

    labels = [[None, 0]] * len(y_test)
    for preds in all_preds:
        for i in range(len(preds)):
            label, percent = preds[i]
            if labels[i][0] != label:
...
ml_classifiers = ['svm']
main(ml_classifiers, train_batch=10, test_batch=10, preprocess_data=False,  train_model=True, train_type='batch', test_type='batch',
     train_subsetn=50, test_data=False)
